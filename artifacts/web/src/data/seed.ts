import type { Audit, CountItem, Locality, Product, StorageLocation, StoredUser, Warehouse } from "./types";

export const SEED_LOCALITIES: Locality[] = [
...
export const SEED_STORAGE_LOCATIONS: StorageLocation[] = [
...
/**
 * Deterministic pseudo-count so the seed looks identical on every machine
 * (the mobile app used Math.random, which produced different demo data each run).
 */
function seededOffset(seed: number): number {
  return (Math.abs(Math.sin(seed) * 10000) % 6) - 3;
}

export function buildSeedCountItems(): CountItem[] {
  const items: CountItem[] = [
    ...buildCountItems("a1", SEED_PRODUCTS.slice(0, 7), ["u1", "u2"]),
    ...buildCountItems("a2", SEED_PRODUCTS.slice(4, 9), ["u1"]),
    ...buildCountItems("a3", SEED_PRODUCTS.slice(12, 15), ["u2"]),
    ...buildCountItems("a4", SEED_PRODUCTS.slice(3, 5).concat(SEED_PRODUCTS.slice(8, 10)), ["u1"]),
    ...buildCountItems("a5", SEED_PRODUCTS.slice(10, 12), []),
  ];

  items.forEach((item, idx) => {
    if (item.auditId === "a2" || item.auditId === "a4") {
      item.countedQty = item.systemQty + Math.round(seededOffset(idx + 1));
      item.status = item.auditId === "a4" ? "aprobado" : "enviado";
      item.countedAt = "2026-06-29T11:00:00Z";
    }
    if (item.auditId === "a1" && idx % 5 < 3) {
      item.countedQty = item.systemQty + Math.round(seededOffset(idx + 100) / 1.5);
      item.status = "contado";
