import type { Audit, CountItem, DashboardMetrics, Product, User } from "@/data/types";

const pct = (num: number, den: number) => (den > 0 ? Math.round((num / den) * 1000) / 10 : 0);

/**
 * Client-side mirror of the server's /metrics aggregation. Used by the offline
 * store and the dashboard so both online and offline modes show identical KPIs.
 */
export function computeMetrics(
  ...
  const auditsByStatus = Array.from(statusCounts.entries()).map(([status, count]) => ({
